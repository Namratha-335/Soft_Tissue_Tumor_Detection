...
def generate_gradcam(model, img_array, class_idx):
    try:
        # Try to get the base model (first layer should be MobileNetV2)
        base_model = None
        for layer in model.layers:
            if hasattr(layer, 'layers'):  # This is a Model object
                base_model = layer
                break
        
        if base_model is None:
            # If no nested model, use the main model
            base_model = model
        
        # Find last conv layer - try common MobileNetV2 layer names
        ...
        # If still not found, search backwards through all layers
...
def main():
    ...
    if model is None:
        st.error("⚠️ Model not found! Please ensure 'tumor_transfer_model.h5' is in the same directory.")
        st.stop()
            ...
            with st.spinner("Analyzing MRI scan..."):
                ...
                with st.spinner("Generating heatmap..."):
                    try:
                        ...
                        st.info("""
                        **Note:** Grad-CAM visualization requires access to the model's convolutional layers. 
                        This might not work if the model was saved in a way that doesn't preserve the layer structure.
                        
                        The prediction results above are still valid and accurate!
                        """)
    ...
    st.markdown("""
        <div style='text-align: center; color: #666; padding: 1rem;'>
            <p>🏥 Soft Tissue Tumor Detection System | Powered by TensorFlow & Streamlit</p>
